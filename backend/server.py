...
Réponds en français de manière professionnelle et détaillée."""
    ...
    chat = LlmChat(
        api_key=EMERGENT_LLM_KEY,
        session_id=f"backtest_{user['id']}_{backtest_id[:8]}",
        system_message=strategy_context
    ).with_model("openai", "gpt-5.2")
    
    try:
        analysis = await chat.send_message(UserMessage(text="Analyse cette stratégie de trading pour le backtesting."))
...
async def get_backtests(user: dict = Depends(get_current_user)):
    ...
    backtests = list(backtests_collection.find(
        {"user_id": user["id"]},
        {"ai_analysis": 0}  # Exclude long analysis from list
    ).sort("created_at", -1))
...
@app.post("/api/backtest/{backtest_id}/trades")
...
@app.post("/api/backtest/{backtest_id}/calculate")
...
Période: {backtest['start_date']} à {backtest['end_date']}
...
- ROI: {results['roi']}%
- Capital: {results['initial_capital']}€ → {results['final_capital']}€
...
1. **Verdict global** - Cette stratégie est-elle viable ? (Oui/Non/Avec ajustements)
2. **Points positifs** des résultats
3. **Points d'alerte** (risques, drawdown, etc.)
4. **Comparaison** avec les standards du marché
5. **Recommandations** pour améliorer la performance
6. **Prochaines étapes** concrètes

Sois direct et actionnable. Réponds en français."""

    chat = LlmChat(
        api_key=EMERGENT_LLM_KEY,
        session_id=f"backtest_results_{user['id']}_{backtest_id[:8]}",
        system_message=analysis_context
    ).with_model("openai", "gpt-5.2")
    
    try:
        performance_analysis = await chat.send_message(UserMessage(text="Analyse ces résultats de backtest."))
        
        results["ai_performance_analysis"] = performance_analysis
        ...
        # Save results even without AI analysis
        backtests_collection.update_one(
            {"_id": backtest_id},
            {"$set": {
                "results": results,
...
@app.delete("/api/backtest/{backtest_id}/trades/{trade_id}")
